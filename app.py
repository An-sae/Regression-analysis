...
if _inst_settings:                     # för administratören, syns i serverloggen
    print(f"[Method Comparison Tool] Inställningsfil saknas: {_inst_settings}", file=sys.stderr)
if _inst_missing:
...
try:
    ...
    from ui.two_file import two_file_sidebar, overview_panel
...
_lf_matched_xlsx=None; _lf_analyte="ALL"; _lf_ctx=None
...
with st.sidebar:
    ...
    if analysis_type == "Deming":
        with st.expander(t("⚙️ Deming options"), expanded=True):
            ...
            error_ratio = st.number_input(
                ...
                help=t("λ = 1 means both methods have equal imprecision."))
    else:
    ...
    if analysis_type == "Precision Evaluation (EP15-A3)":
        ...
        if _lf_mode=="Two long-format files (match by ID)":
            ...
            _lf_label_a, _lf_label_b = _tf["label_a"], _tf["label_b"]
    ...
    st.caption(f"**{version_string()}** · " + t("validated {d}").format(d=VALIDATED_ON))
    ...
    with st.expander(t("🛈 System status")):
        if not _inst_changed and not _inst_settings and _inst_version:
            ...
            st.caption(t("No manifest.json found; the installation cannot be checked."))
        if _inst_changed:
            st.caption(t("Differs from the validated version: {f}").format(f=", ".join(_inst_changed)))
        if _inst_settings:
            st.caption(t("The settings file .streamlit/config.toml is missing. Calculations are "
